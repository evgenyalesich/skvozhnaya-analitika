# SQL-построитель основного Roistat weekly-запроса (разбивка по company × week_start).
# CTE: first_seen → cohort? → start_rows → entered_week_metrics → lead_rows → user_flags → budget_rows.
# Группирует по DATE_TRUNC('week') + normalized_company, считает ~30 метрик воронки.

from collections.abc import Callable
from typing import Any


def build_main_weekly_query(
    *,
    ...
    SELECT
        ...
        COALESCE(scs.new_in_system, 0) AS new_in_system,
        COALESCE(scs.old_in_system, 0) AS old_in_system,
        ...
        COALESCE(cm.refused_interview, 0) AS refused_interview,
        COALESCE(cm.no_response_interview, 0) AS no_response_interview,
    ...
    LEFT JOIN start_company_segments scs ON scs.week_start = cw.week_start AND scs.company = cw.company
