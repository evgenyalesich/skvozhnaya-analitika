...
- Для рекламных метрик используется `GET /api/reports/budgets/weekly` с `interval=day`.
- Значения недельных бюджетов и метрик распределяются по дням, затем агрегируются по выбранному диапазону.
- Для формул используется `spend_base`:
  - если `spend > 0`, берем `spend`.
  - иначе берем `budget`.
